...
class StringWriter:
    ...
    def h2(self, content, _id='', blank=True):
        """Write an h2 element.
        ...
        """
        if _id:
            return self.write(f'<h2 id="{_id}">{content}</h2>', blank=blank)
        else:
            return self.write(f'<h2>{content}</h2>', blank=blank)
    
    def h3(self, content, blank=True):
        """Write an h3 element.

        >>> StringWriter().h3('testing').text.strip()
        '<h3>testing</h3>'
        """
